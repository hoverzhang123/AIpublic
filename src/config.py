...
class Config:
    ...
    def __init__(
        ...
        max_retrievals: Optional[int] = None,
        ...
        object.__setattr__(
            self,
            "milvus_uri",
            milvus_uri or os.getenv("MILVUS_URI", "http://localhost:19530"),
        )
        object.__setattr__(
            self,
            "collection_name",
            collection_name or os.getenv("MILVUS_COLLECTION", "my_rag_collection"),
        )
        object.__setattr__(
            self,
            "openai_api_key",
            openai_api_key or os.getenv("LOCAL_API_KEY") or os.getenv("OPENAI_API_KEY"),
        )
        object.__setattr__(
            self,
            "openrouter_model",
            openrouter_model
            or os.getenv("OPENROUTER_MODEL", "deepseek/deepseek-v4-flash:free"),
        )
        object.__setattr__(
            self,
            "faq_path",
            faq_path or Path(os.getenv("MILVUS_DOCS_PATH", "src/milvus_docs/en/faq")),
        )
        object.__setattr__(
            self,
            "max_retrievals",
            max_retrievals or int(os.getenv("MAX_RETRIEVALS", "3")),
        )
        object.__setattr__(
            self, "metric_type", metric_type or os.getenv("MILVUS_METRIC", "IP")
        )
        object.__setattr__(
            self,
            "consistency_level",
            consistency_level or os.getenv("MILVUS_CONSISTENCY", "Strong"),
        )
        object.__setattr__(
        ...
        object.__setattr__(
            self,
            "llm_base_url",
            llm_base_url or os.getenv("LLM_BASE_URL", "https://openrouter.ai/api/v1"),
        )
